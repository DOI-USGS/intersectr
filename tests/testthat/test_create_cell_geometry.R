...
test_that("crossing date line more", {
  ...
  geom <- st_as_sfc("POLYGON ((-173.1318 51.95922, -179.1185 51.15853, -179.1336 51.15695, -179.1488 51.1572, 179.226 51.29705, 179.2137 51.29861, 173.6277 52.29857, 173.3859 52.33802, 173.317 52.35366, 173.3046 52.35788, 173.2945 52.36279, 172.3714 52.88758, 172.3603 52.89557, 172.3528 52.90398, 172.3477 52.91396, 172.3462 52.92348, 172.3474 52.93215, 172.3506 52.93974, 172.3565 52.94769, 172.3645 52.95491, -173.1305 60.73862, -152.0446 64.41658, -151.5654 64.37961, -151.5436 64.37583, -151.5136 64.35263, -151.512 64.34964, -151.5105 64.31368, -151.6557 63.02242, -151.6715 62.93349, -152.7324 60.74787, -154.3472 58.45999, -154.3605 58.44583, -159.1322 54.87275, -159.1487 54.85934, -159.1973 54.82705, -159.21 54.82081, -159.4758 54.71403, -159.489 54.70982, -159.506 54.70642, -169.2089 52.71876, -169.2415 52.7118, -173.1318 51.95922))",
                    crs = 4326)
  warn <- capture_warnings(cells <- create_cell_geometry(x, y, 4326, geom, regularize = TRUE))
  expect(nrow(cells) == 1872)
  # prj <- "+proj=aea +lat_1=55 +lat_2=65 +lat_0=50 +lon_0=-154 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs"
  # plot(st_transform(geom, prj))
  # plot(st_transform(cells$geometry, prj), add = TRUE)
})
